...
object DeferredAcceptance {
  ...
  /** Compute a weakly stable matching between two sets of equal size.
    ...
    */
  def weaklyStableMatching[M <: Proposer with Preferences[W], W <: Preferences[M]]: State[(Set[M], Set[W]), Map[W, M]] = {
  ...
  /** Compute a weakly stable matching between two sets.
    ...
    *       is the size of the inputs sets.
    */
  def weaklyStableMatching2[M <: Proposer with Predicate[W] with Preferences[W], W <: Predicate[M] with Preferences[M]]
                           : State[(Set[M], Set[W]), Map[W, M]] = {
